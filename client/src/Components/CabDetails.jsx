...
import axios from 'axios';
import {
  Star, ArrowLeft, Calendar, Home, Navigation
} from 'lucide-react';
import Navbar from './Navbar';
import Footer from './Footer';
        ...
        <div className="w-full mx-auto px-6 lg:px-[4rem] py-[5rem]">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
              <div className="relative h-96 lg:h-auto">
                <img
                  src={cab.image || 'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d'}
                  alt={cab.name}
                  className="w-full h-full object-cover"
                />
                <div className="absolute top-4 right-4 bg-white/90 px-3 py-1 rounded-full shadow-sm flex items-center">
                  <Star className="w-4 h-4 fill-yellow-400 text-yellow-400 mr-1" />
                  <span className="font-medium text-gray-800">{cab.rating?.toFixed(1) || '4.5'}</span>
                </div>
              </div>
              ...
              <div className="p-6 md:p-8 lg:p-10">
                ...
                <div className="flex flex-col h-full">
                  <div className="mb-6">
                    <div className="flex items-start justify-between">
                      <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{cab.name}</h1>
                        <div className="flex items-center mt-2 space-x-2">
                          <span className="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{cab.model}</span>
                          <span className="text-sm text-gray-500">• {cab.seats || 4} seats</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-3xl font-bold text-green-600">₹{cab.price}</p>
                  ...
                  <div className="mb-6 space-y-3">
                    <div className="flex items-start">
                      ...
                      <div className="ml-2">
                        <p className="text-gray-800 font-medium">{cab.address?.city}, {cab.address?.state}</p>
                        <p className="text-gray-500 text-sm">{cab.address?.fullAddress || 'Address not specified'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['AC', 'WiFi', 'Charging Port', 'GPS', 'Child Seat'].map((feature) => (
                        <span key={feature} className="bg-gray-100 text-gray-800 text-sm font-medium px-3 py-1 rounded-full">{feature}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="mb-6 flex-grow">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Description</h3>
                    <p className="text-gray-600">
                      {cab.description || 'Comfortable cab with all modern amenities.'}
                    </p>
                  </div>
