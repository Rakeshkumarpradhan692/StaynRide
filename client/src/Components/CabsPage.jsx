...
export default function CabsPage() {
    ...
    const fetchCabs = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/public/all-cabs"
        );
  ...
  const filteredCabs = cabs.filter((cab) => {
    const nameMatch =
      !searchName || cab.name.toLowerCase().includes(searchName.toLowerCase());
    const stateMatch = !selectedState || cab.address?.state === selectedState;
    const cityMatch = !selectedCity || cab.address?.city === selectedCity;
    const typeMatch =
      !selectedType || cab.name.toLowerCase() === selectedType.toLowerCase();
    const capacityMatch =
      !selectedCapacity ||
  ...
  const uniqueStates = [
    ...new Set(cabs.map((cab) => cab.address?.state).filter(Boolean)),
  ];
  const uniqueCities = [
    ...new Set(cabs.map((cab) => cab.address?.city).filter(Boolean)),
  ];
